...
function consistent_mcc_triplets(M12, M13, M23, trees)
	...
	for n in nodes(trees[1])
		if TreeKnit.is_branch_in_mccs(n, M12) && TreeKnit.is_branch_in_mccs(n, M13)
			Z += 1
			...
			# belongs to
			m12 = TreeKnit.find_mcc_with_branch(n, M12)[2]
			m13 = TreeKnit.find_mcc_with_branch(n, M13)[2]
			clade = [x.label for x in POTleaves(n)]
			n2 = lca(trees[2], intersect(clade, m12))
			n3 = lca(trees[3], intersect(clade, m13))
			if !TreeKnit.is_branch_in_mccs(n2, M23) || !TreeKnit.is_branch_in_mccs(n3, M23)
				s += 1
			end
		end
	end
